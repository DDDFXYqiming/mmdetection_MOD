...
class YOLOXHead(BaseDenseHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.multi_level_kp1_convs = nn.ModuleList()
        self.multi_level_kp2_convs = nn.ModuleList()
        ...
        for _ in self.strides:
            ...
            self.multi_level_kp1_convs.append(self._build_stacked_convs())  # 构建定位卷积层
            self.multi_level_kp2_convs.append(self._build_stacked_convs())  # 构建定位卷积层
    ...
    def forward_single(
        ...
        kp1_feat = kp1_convs(x)
        kp2_feat = kp2_convs(x)

        cls_score = conv_cls(cls_feat)  # 分类预测
        bbox_pred = conv_reg(reg_feat)  # 定位预测
        objectness = conv_obj(reg_feat)  # 目标性预测

        kp1_pred = conv_kp1(kp1_feat)  # 第一个关键点预测
        kp2_pred = conv_kp2(kp2_feat)  # 第二个关键点预测
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        return multi_apply(
            ...
            self.multi_level_kp1_convs,
            self.multi_level_kp2_convs,
    ...
    def decode_keypoints(self, priors: Tensor, keypoint_preds: Tensor) -> Tensor:
        """将关键点的预测结果解码为实际的关键点坐标。

        Args:
            priors (Tensor): 锚框，形状为 (num_instances, 2)。
            keypoint_preds (Tensor): 所有实例的关键点预测结果，形状为 (batch_size, num_keypoints, 2)。

        Returns:
            Tensor: 解码后的关键点坐标，形状为 (batch_size, num_keypoints, 2)。
        """
        decoded_keypoints = (keypoint_preds * priors[:, 2:]) + priors[:, :2]
    ...
    def loss_by_feat(
        ...
        if num_pos > 0:
            ...
            # # 新增关键点的损失计算
            # loss_kp1 = (
            #     self.loss_kp(flatten_kp1.view(-1, 2)[pos_masks][valid_mask], kp1_targets[valid_mask])
            #     / (valid_mask.sum().item()+ 1e-8)
            # )
            # loss_kp2 = (
            #     self.loss_kp(flatten_kp2.view(-1, 2)[pos_masks][valid_mask], kp2_targets[valid_mask])
            #     / (valid_mask.sum().item()+ 1e-8)
            # )
            ...
            # 筛选有效的关键点坐标和预测值
            filtered_kp1_preds = flatten_kp1.view(-1, 2)[pos_masks][valid_mask]
            filtered_kp1_targets = kp1_targets[valid_mask]

            # print(kp1_targets[valid_mask])
            
            filtered_kp2_preds = flatten_kp2.view(-1, 2)[pos_masks][valid_mask]
            filtered_kp2_targets = kp2_targets[valid_mask]

            # 计算偏移量
            offset_kp1 = filtered_kp1_preds - filtered_kp1_targets
            offset_kp2 = filtered_kp2_preds - filtered_kp2_targets

            # 计算偏移量的L1损失
            loss_kp1 = offset_kp1.abs().sum()/ (valid_mask.sum().item()+ 1e-8)
            loss_kp2 = offset_kp2.abs().sum()/ (valid_mask.sum().item()+ 1e-8)
