...
class YOLOXHead1(BaseDenseHead):
    ...
    def _build_predictor(self) -> Tuple[nn.Module, nn.Module, nn.Module]:
        ...
        conv_reg = nn.Conv2d(self.feat_channels, 8, 1)  # 定位预测
    ...
    def predict_by_feat(
        ...
        flatten_bboxes, flatten_keypoints_preds = self._bbox_decode(flatten_priors, flatten_bbox_preds)
        ...
        for img_id, img_meta in enumerate(batch_img_metas):
            ...
            results = InstanceData(
                ...
                keypoints=flatten_keypoints_preds[img_id][valid_mask],  # Add keypoints to results
    ...
    def _bbox_decode(self, priors: Tensor, bbox_keypoints_preds: Tensor) -> Tuple[Tensor, Tensor]:
        """将回归和关键点预测结果解码为边界框和关键点坐标。

        Args:
            priors (Tensor): 图像的中心priors，形状为(num_instances, 2)。
            bbox_keypoints_preds (Tensor): 所有实例的盒子能量/偏差和关键点预测，形状为(batch_size, num_instances, 8)。

        Returns:
            Tuple[Tensor, Tensor]: 解码后的边界框和关键点坐标。
        """
        # Split bbox and keypoints predictions
        bbox_preds = bbox_keypoints_preds[..., :4]
        keypoints_preds = bbox_keypoints_preds[..., 4:]

        # Bbox decoding as before
        ...
        return decoded_bboxes, keypoints_preds
    ...
    def _get_targets_single(
        ...
        if num_gts == 0:
            ...
            bbox_target = cls_preds.new_zeros((0, 8))
        ...
        bbox_target = torch.cat([sampling_result.pos_gt_bboxes, keypoints_targets], dim=1)
