            ...
            if key == "gt_masks" or isinstance(results[key], BaseBoxes):
                ...
                if "gt_ignore_flags" in results:
                    try:
                        ...
                        # print("原数据")
                        # print(results[key][valid_idx])
                    except:
                        # print("数据")
                        # print(valid_idx)
                        num_kps = 2
                        dummy_keypoints = np.zeros((len(valid_idx), num_kps, 3), dtype=np.float32)  # 生成替代数据
                        instance_data[self.mapping_table[key]] = to_tensor(dummy_keypoints)
                else:
