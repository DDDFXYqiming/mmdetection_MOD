...
class Resize(MMCV_Resize):
    ...
    def _resize_keypoints(self, results: dict) -> None:
        ...
        if results.get('gt_keypoints', None) is not None:
            ...
            # Apply scaling to keypoints coordinates
            keypoints[:, :, :2] *= results['scale_factor']

            if self.clip_object_border:
                # Clip keypoints coordinates to image boundaries
...
class MixUp(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        retrieve_gt_keypoints = retrieve_results.get('gt_keypoints', None)
        if retrieve_gt_keypoints is not None:
            retrieve_gt_keypoints[:, :, :2] *= scale_ratio
            if is_filp:
                retrieve_gt_keypoints[:, :, 0] = target_w - retrieve_gt_keypoints[:, :, 0]
            retrieve_gt_keypoints[:, :, :2] -= [x_offset, y_offset]
            if self.bbox_clip_border:
                retrieve_gt_keypoints[:, :, 0] = np.clip(
                    retrieve_gt_keypoints[:, :, 0], 0, target_w - 1)
                retrieve_gt_keypoints[:, :, 1] = np.clip(
                    retrieve_gt_keypoints[:, :, 1], 0, target_h - 1)
        ...
        cp_retrieve_gt_keypoints = retrieve_gt_keypoints.copy()
        cp_retrieve_gt_keypoints[:, :, :2] -= [x_offset, y_offset]
        if self.bbox_clip_border:
            cp_retrieve_gt_keypoints[:, :, 0] = np.clip(
                cp_retrieve_gt_keypoints[:, :, 0], 0, target_w - 1)
            cp_retrieve_gt_keypoints[:, :, 1] = np.clip(
                cp_retrieve_gt_keypoints[:, :, 1], 0, target_h - 1)
        ...
        mixup_gt_keypoints = np.concatenate(
            (results.get('gt_keypoints', None), cp_retrieve_gt_keypoints), axis=0)
...
class RandomAffine(BaseTransform):
    ...
    def _apply_keypoint_affine(self, keypoints, warp_matrix, border, height, width):
        new_keypoints = np.zeros_like(keypoints)
        for i, keypoints_per_person in enumerate(keypoints):
            new_keypoints_per_person = np.zeros_like(keypoints_per_person)
            for j, keypoint in enumerate(keypoints_per_person):
                x, y, v = keypoint
                if v <= 0:
                    new_keypoints_per_person[j] = keypoint
                    continue
                new_keypoint = np.dot(warp_matrix, np.array([x, y, 2]))
                new_x, new_y, new_v = new_keypoint
                new_x = np.clip(new_x, 0, width - 1)
                new_y = np.clip(new_y, 0, height - 1)
                new_keypoints_per_person[j] = [new_x, new_y, new_v]
            new_keypoints[i] = new_keypoints_per_person
    ...
    def transform(self, results: dict) -> dict:
        ...
        if keypoints is not None:
            ...
            new_keypoints = self._apply_keypoint_affine(
                keypoints, warp_matrix, self.border, height, width)
